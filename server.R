...
shinyServer(
     function(input, output) {
          output$risk1 <- renderText(ASCVD(input$gender,input$race,input$treated,input$age,
                               input$tchol,input$hdl,input$sbp,input$smoking,input$dm))
          output$risk2 <- renderText(ASCVD(input$gender, input$race, 0, input$age,
                                           170, 50, 110, 0, 0))
     }
)